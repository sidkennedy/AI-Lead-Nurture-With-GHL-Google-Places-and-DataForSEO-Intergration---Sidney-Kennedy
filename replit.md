...
- G — D's Step 1 opener ("probably won't work for your practice") + B's full flow from Step 2 onward (starts disabled)
...
**Same trap, different shape — `\n` (or `\t`) inside a JS string literal inside the outer backtick template gets rendered as a real newline/tab:** Inside a backtick template literal in server.js, any escape sequence like `\n` is interpreted by Node as a real newline character. So writing `qi.text.split('\n\n')` inside the admin page template outputs a literal two-line-broken string in the browser, which the browser JS parser sees as an unterminated string literal → SyntaxError — the entire admin script fails to parse and EVERY panel shows "Loading…" forever. This crashed the admin dashboard on 2026-05-01 (line 5351, `qi.text.split('\n\n')` in the AI Analysis block). **Hard rule:** any string escape sequences (`\n`, `\t`, `\r`, etc.) that should appear literally in the output JavaScript must be double-escaped as `\\n`, `\\t`, `\\r` when written inside the server.js page template. The binary-search parse test above will catch this.
