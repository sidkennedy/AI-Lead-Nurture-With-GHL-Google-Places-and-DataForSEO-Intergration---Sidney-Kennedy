...
**Explicit handoff carve-out (added Apr 28, Carson regression):** the qualification gate is bypassed when the prospect's most recent inbound matches a handoff-signal regex — the WANT-VIDEO-NOW phrases ("just send the link", "fire it over", "give me the video/training/program/meeting", "show me it") or the WANT-HUMAN phrases ("talk to a human/person/someone", "I don't want to talk to AI", "chat bot", "stop the bot", "no more bot", "give me a call"). When the regex matches AND `[BOOKED]` is emitted, the marker is honored even without research/scan in context — the contact is marked `booked=true, paused_reason='verbal-commit'` as normal and Sid takes the handoff. Logs `[AiGen] [BOOKED] allowed via handoff carve-out for ${contactId} (matched: ...)` for audit. Mirrors the prompt-level WANT-VIDEO-NOW + WANT-HUMAN handlers under `OFF-SCRIPT REPLIES` (added in the same patch to all 5 prompts — canonical + A/B/C/D — `data/prompts.json` + `config.js` seed, VERSION 6 → 7). The standalone `━━━ EARLY BOOKING ━━━` block was retired in the same patch (functionally subsumed by WANT-VIDEO-NOW). Trigger lists in the prompts and the regex list in `server.js` are intentionally generous — false positives just mean a slightly eager handoff, which is the entire point of the new handlers. Carson trigger that motivated the carve-out: contact `sYVGEfUpY5GMHJYa7odh` saying "Just give me the training program or meeting. I do not want to talk to an AI chat about. Chat bot" — pre-patch the bot replied with another info-gathering ask + video tease (SOLUTION-SEEKING handler intercepted); post-patch the same input now gets the VSL link + Sid-takes-over reassurance + `[BOOKED]`. **Follow-up gotcha discovered during stress-testing the patch:** the original `NEVER BOOK BEFORE QUALIFYING` block listed inline examples ("yes send me the video", "fire it over", "send the link", "I want to watch it") that overlap word-for-word with the new WANT-VIDEO-NOW handler triggers. Even with the `EXCEPTION — explicit handoff carve-out` paragraph appended at the bottom of the same block, the model preferred the more concrete inline examples and pivoted to the qualification gate instead of firing `[BOOKED]`. The fix was to replace those video-asking examples with vague-enthusiasm phrases ("yes", "sure", "yep", "sounds good", "I'm in", "let's do it") plus an explicit NOTE pointing handler-overlapping phrases at WANT-VIDEO-NOW / WANT-HUMAN. **Invariant for future edits to this block:** never name a phrase in the inline examples that ALSO appears as a trigger in any `OFF-SCRIPT REPLIES` handler — pick examples for one rule or the other, not both. Validated by `/tmp/test_fire_over.js` (3/3 "fire it over" cases PASS post-fix on variants A/B/C; 0/3 "yes" cases fired `[BOOKED]`, confirming the trap #8 invariant still holds for vague mid-funnel acknowledgements). Anthropic's per-org 30K input-tokens-per-minute rate limit means parallel testing of all 4 variants requires throttling — the targeted regression script uses 16s spacing between calls.

**Three-way precedence for STOP / bot / human inputs (post-architect-review refinement):** the inbound flow has THREE intent classifiers that can match overlapping phrases — **opt-out** (`optouts.isOptOutKeyword(messageBody)` at `server.js:451`, regex `\b(stop|unsubscribe|quit|cancel|end|optout|opt[ -]out)\b`) → **server-side handoff carve-out** (`HANDOFF_REGEXES` in `handleInbound` ~line 1398) → **prompt-level OFF-SCRIPT REPLIES handlers** (WANT-HUMAN, OBJECTIONS "Is this a bot?"). The order is fixed: opt-out runs first in the webhook handler and short-circuits the entire conversation if it matches, so it ALWAYS wins for any message containing a TCPA opt-out keyword (`"stop the bot"` → opt-out, never reaches AI). For this reason the WANT-HUMAN trigger lists in the prompts and the `HANDOFF_REGEXES` list in `server.js` intentionally **exclude** "stop the bot" / "stop the chatbot" — TCPA-compliance demands STOP win even when the user is wrapping it in bot-rejection language. The remaining ambiguity is "are you a bot?" vs "no more bot": the OBJECTIONS handler "Is this a bot?" carries an inline NOTE telling the model to deflect ONLY for curiosity questions ("Is this a bot?", "are you a bot?") and to defer to WANT-HUMAN for explicit rejections ("no more bot", "I don't want to talk to a bot", "I don't want to talk to AI", "chat bot" used as a complaint). `HANDOFF_REGEXES` mirrors this — it includes `chat\s?bot`, `no\s+more\s+(?:chat\s?)?bot`, `kill\s+(?:the\s+)?(?:chat\s?)?bot`, `(?:i\s+)?(?:don'?t|do\s+not)\s+want\s+(?:to\s+talk\s+to\s+)?(?:an?\s+)?(?:chat\s?)?bot`, and `(?:i\s+)?(?:don'?t|do\s+not)\s+want\s+(?:to\s+talk\s+to\s+)?(?:an?\s+)?ai`, but NOT `are\s+you\s+a\s+bot` (curiosity), and NOT `stop\s+(?:the\s+)?bot` (opt-out). Smoke-tested by `/tmp/test_handoff.js` regex sub-suite: 17 of 18 routing assertions pass; the one "stop the chatbot → handoff=true" hit is benign because opt-out fires first in production. **Invariant for future trigger-list edits:** any new WANT-HUMAN phrase must be checked against the opt-out keyword list and the OBJECTIONS list before adding — three classifiers, one source of truth missing, so the diff has to be manually cross-referenced.
